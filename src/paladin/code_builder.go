...
func NewCodeBuilder(fileName string) *CodeBuilder {
	c := new(CodeBuilder)
...
func (p *CodeBuilder) genInit() {
	p.jfile.Func().Id("init").Params().Block(
		jen.List(jen.Id("file"), jen.Id("err")).Op(":=").Qual("os", "Open").Call(jen.Lit("bin/dbc/location.json")),
...
// 输出
func (p *CodeBuilder) Output(dir string) {
	if err := p.jfile.Save(dir + "/" + p.fileName); err != nil {
		plog.Error(err)
	}
}
